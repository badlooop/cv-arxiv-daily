...
- **2025-11-26** **G $^2$ VLM: Geometry Grounded Vision Language Model with Unified 3D Reconstruction and Spatial Reasoning** [2511.21688](http://arxiv.org/abs/2511.21688)
  > 视觉语言模型（VLM）在空间智能方面仍然缺乏鲁棒性，在空间理解和推理任务上表现不佳。我们将这一差距归因于缺乏能够从 2D 图像重建 3D 空间的视觉几何学习过程。我们提出了 G $^2$VLM，一种基于几何的视觉语言模型，它连接了空间智能的两个基本方面：空间 3D 重建和空间理解。 G$^2$VLM 本身利用学习的 3D 视觉几何特征来直接预测 3D 属性，并通过上下文学习和交错推理来增强空间推理任务。我们的统一设计对于空间理解具有高度可扩展性：它训练丰富的多视图图像和视频数据，同时利用通常仅源自难以收集的注释的 3D 视觉先验的优势。实验结果表明，G$^2$VLM 精通这两项任务，取得了与最先进的前馈 3D 重建模型相当的结果，并在空间理解和推理任务中取得了更好或有竞争力的结果。通过将语义上强大的 VLM 与低级 3D 视觉任务相结合，我们希望 G$^2$ VLM 可以作为社区的强大基线，并解锁更多未来的应用程序，例如 3D 场景编辑。

- **2025-11-26** **Holographically Emergent Gauge Theory in Symmetric Quantum Circuits** [2511.21685](http://arxiv.org/abs/2511.21685)
  > 我们为随机量子电路中的混合态相（酉和非酉）开发了一种新颖的全息框架，具有全局对称性 $G$。将电路视为张量网络，我们将其分解为两部分：对称层，定义一个更高维度的涌现规范波函数，以及随机非对称层，由随机多重张量组成。对于酉电路，体积规范状态是解除限制的，但在一般的非酉电路（例如通道）下，体积规范理论可以经历退相干引起的相变：对于具有局部对称噪声的 $G\,{=}\,\mathbb{Z}_N$ ，电路可以充当量子纠错码，具有继承 $\mathbb{Z}_N$ 表面代码的拓扑保护的独特逻辑子空间。然后，我们发现来自测量侧的电荷锐化转变与体中的可解码性转变是互补的：体中的噪声可以被解释为来自环境的测量。对于 $N\,{\leq}\,4$，弱测量驱动从具有锐化时间 $t_{\#}\sim e^{L}$ 的电荷模糊阶段到具有 $t_{\#}\sim \mathcal{O}(1)$ 的电荷锐化阶段的单个转变，对应于破坏逻辑信息的限制。对于 $N>4$，测量通常会生成具有无间隙光子和净化时间 $t_{\#}\sim \mathcal{O}(L)$ 的中间准长程有序库仑相。

- **2025-11-26** **Fast 3D Ultrasound Localization Microscopy via Projection-based Processing Framework** [2511.21647](http://arxiv.org/abs/2511.21647)
  > 三维超声定位显微镜 (ULM) 可实现脉管系统的全面可视化，从而提高诊断可靠性。然而，其临床转化仍然具有挑战性，因为全 3D 重建的体素数量呈指数增长，带来了大量的计算需求和大量的后处理时间。在这项基于行列阵列 (RCA) 的 3D 体内猪肾 ULM 研究中，我们重新制定了完整 3D ULM 流程的每个步骤，包括波束形成、杂波过滤、运动估计、微泡分离和定位到一系列计算高效的 2D 操作中，大大减少了要处理的体素数量，同时保持了相当的精度。所提出的框架在单个 RTX A6000 Ada GPU 上在 0.52 秒（采集时间的 70%）内重建以 400 Hz 帧速率采集的每个 0.75 秒整体，覆盖 25*27.4*27.4 mm3 体积，同时保持与传统 3D 处理相当的 ULM 图像质量。从数量上讲，它在密度图之间实现了 0.93 的结构相似性指数 (SSIM)，斜率为 0.93 且 R2 = 0.88 的体素速度一致性，与传统 3D 结果紧密匹配，并首次展示了扫描过程中实时反馈的潜力，这可以提高鲁棒性，减少操作员依赖性并加速临床工作流程。

- **2025-11-26** **Diagonal Scaling: A Multi-Dimensional Resource Model and Optimization Framework for Distributed Databases** [2511.21612](http://arxiv.org/abs/2511.21612)
  > 现代云数据库将扩展呈现为二元决策：通过添加节点进行扩展，或通过增加每个节点的资源进行扩展。这种一维视图是有局限性的，因为数据库性能、成本和协调开销是由水平弹性和每节点 CPU、内存、网络带宽和存储 IOPS 的共同相互作用产生的。因此，系统经常对负载峰值反应过度，对内存压力反应不足，或者在次优状态之间振荡。我们引入了缩放平面，这是一个二维模型，其中每个分布式数据库配置都表示为一个点（H，V），其中 H 表示节点数，V 表示资源向量。在这个平面上，我们定义了延迟、吞吐量、协调开销和货币成本的平滑近似值，提供了性能权衡的统一视图。我们通过分析和经验表明，最佳缩放轨迹通常位于对角路径上：同时利用集群并行性和每个节点改进的联合水平和垂直调整序列。为了计算此类动作，我们提出了 DIAGONALSCALE，这是一种离散局部搜索算法，用于评估缩放平面中的水平、垂直和对角线移动，并选择最小化受 SLA 约束的多目标函数的配置。使用合成表面、微基准测试以及分布式 SQL 和 KV 系统上的实验，我们证明，与仅水平和仅垂直自动缩放相比，对角缩放可将 p95 延迟减少多达 40%，将每次查询成本降低多达 37%，并将重新平衡减少 2 到 5 倍。我们的结果强调了对多维扩展模型的需求，并为云数据库系统中的下一代自动扩展奠定了基础。
...
- **2025-11-26** **CM Drinfeld Modules, Self-isogenous Modular Polynomials, and Volcano Structure** [2511.21329](http://arxiv.org/abs/2511.21329)
  > 在本文中，我们开发了自同构模多项式的视图以及任意阶 $r$ 的 CM Drinfeld 模的 $\mathfrak{l}$ 循环同构图。在计算方面，我们给出了一个显式的过程来构造阶 $r\geqslant 3$ 的 Drinfeld 模块的模多项式 $Φ_{J,\mathfrak{a}}(X,X)$，其中 $\mathfrak{a}$ 是 $\mathbb{F}_q[T]$ 的素理想。当$\mathfrak{a}=(T)$时，我们提供一个算法来计算$Φ_{J,\mathfrak{a}}(X,X)$；当$\mathfrak{a}=(T^2+T+1)$时，我们在$Φ_{J,\mathfrak{a}}(X,X)$上给出明确的度数界限。在结构方面，我们制定了广义$\mathfrak{l}$循环火山结构，并证明广义火山出现在具有复杂乘法的rank-$r$Drinfeld模块的完整$\mathfrak{l}$ 循环同源图的组成部分中。

- **2025-11-26** **HTTM: Head-wise Temporal Token Merging for Faster VGGT** [2511.21317](http://arxiv.org/abs/2511.21317)
  > Visual Geometry Grounded Transformer (VGGT) 标志着 3D 场景重建领域的重大飞跃，因为它是第一个一次性直接联合推断所有关键 3D 属性（相机姿态、深度和密集几何）的模型。然而，这种联合推理机制需要全局注意力层，对来自所有视图的标记执行全面的注意力计算。对于具有长序列输入的大型场景的重建，这会导致显着的延迟瓶颈。在本文中，我们提出了头向时间合并（HTTM），这是一种用于加速 VGGT 的免训练 3D 令牌合并方法。现有的合并技术在不同的注意力头上统一合并标记，导致层的输出中出现相同的标记，这阻碍了模型的表示能力。 HTTM 通过以多头粒度合并标记来解决这个问题，这在头连接后保留了特征标记的唯一性。此外，与现有方法相比，这使得 HTTM 能够利用在头部级别观察到的空间局部性和时间对应性，以更低的合并成本实现更高的合并率。因此，HTTM 在基于 GPU 的推理中实现了高达 7 倍的加速，且性能下降可以忽略不计。

- **2025-11-26** **Defect-Aware Physics-Based Compact Model for Ferroelectric nvCap: From TCAD Calibration to Circuit Co-Design** [2511.21267](http://arxiv.org/abs/2511.21267)
  > 基于铁电非易失性电容的存储器可实现非破坏性读出和具有 3D 堆叠潜力的低功耗内存计算。然而，它们有限的存储窗口（1-10 fF/μm）需要材料-器件-电路协同优化。现有的紧凑模型无法捕捉小信号电容、器件可变性和循环退化的物理特性，而这些是电路设计的关键参数。在非易失性电容器件中，极化状态的小信号电容差是关键指标。大多数报告的紧凑模型没有包含任何作为电压和极化函数的电容物理模型。我们提出了一种基于物理的紧凑模型，通过结合实验数据、TCAD 模拟和电路验证的多尺度建模来捕获小信号电容、界面和体缺陷贡献以及器件变化。基于这种方法，我们展示了具有 +/- 5 mV 感测裕度的优化存储器读出以及电路级器件耐用性的影响。这项工作提出了一个全面的紧凑模型，可用于内存计算和存储内存的无选择器阵列和 3D 堆栈内存的设计。

- **2025-11-26** **FIELDS: Face reconstruction with accurate Inference of Expression using Learning with Direct Supervision** [2511.21245](http://arxiv.org/abs/2511.21245)
  > 面部表情传达了人类交流中的大量情感信息，但由于依赖 2D 监督且缺乏 3D 地面实况，现有的 3D 面部重建方法经常会错过微妙的情感细节。我们提出了 FIELDS（使用直接监督学习进行精确表情推理的面部重建），通过使用直接 3D 表情参数监督和辅助情感识别分支扩展自监督 2D 图像一致性线索来解决这些限制。我们的编码器以来自自发 4D 面部扫描的真实表情参数为指导，而强度感知情绪损失则鼓励 3D 表情参数捕捉真实的情绪内容而不夸张。这种双重监督策略弥合了 2D/3D 域差距并减轻了表达强度偏差，从而产生保留微妙情感线索的高保真 3D 重建。 FIELDS 从单个图像中生成具有高度真实表情的情感丰富的面部模型，在不牺牲自然度的情况下显着提高了野外面部表情识别性能。

...
- **2026-01-15** **Action100M: A Large-scale Video Action Dataset** [2601.10592](http://arxiv.org/abs/2601.10592)
  > 从视觉观察中推断物理行为是在物理世界中推进机器智能的基本能力。实现这一目标需要跨越广泛领域的大规模、开放词汇的视频动作数据集。我们引入了 Action100M，这是一个由 120 万个互联网教学视频（持续时间 14.6 年）构建的大型数据集，产生 O（1 亿）个时间局部片段，具有开放词汇动作监督和丰富的字幕。 Action100M 由完全自动化的管道生成，该管道 (i) 使用 V-JEPA 2 嵌入执行分层时间分段，(ii) 生成以字幕树形式组织的多级帧和分段字幕，以及 (iii) 在多轮 Self-Refine 程序下使用推理模型 (GPT-OSS-120B) 聚合证据，以输出结构化注释（简短/详细动作、演员、简短/详细字幕）。在 Action100M 上训练 VL-JEPA 在不同的动作识别基准上展示了一致的数据扩展改进和强大的零样本性能，将 Action100M 奠定了视频理解和世界建模可扩展研究的新基础。
